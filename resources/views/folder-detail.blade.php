...
/* ── MODAL CARI OTOMATIS ── */
.search-modal { background: #fff; border-radius: 18px; width: 680px; max-width: 95vw; max-height: 85vh; display: flex; flex-direction: column; box-shadow: 0 24px 64px rgba(0,0,0,0.2); overflow: hidden; }
.search-modal-header { padding: 20px 24px 16px; border-bottom: 1px solid var(--border); flex-shrink: 0; }
.search-modal-title { font-size: 15px; font-weight: 600; margin-bottom: 12px; display: flex; align-items: center; justify-content: space-between; }
.search-input-wrap { display: flex; gap: 8px; }
.search-input { flex: 1; padding: 10px 14px; border: 1.5px solid var(--border); border-radius: 10px; font-family: 'DM Sans', sans-serif; font-size: 13px; outline: none; transition: border-color 0.12s; }
.search-input:focus { border-color: var(--green-border); }
.search-btn { padding: 10px 18px; background: var(--green); color: #fff; border: none; border-radius: 10px; font-family: 'DM Sans', sans-serif; font-size: 13px; font-weight: 600; cursor: pointer; display: flex; align-items: center; gap: 6px; white-space: nowrap; }
.search-btn:hover { background: #14482a; }
.search-btn:disabled { background: #9ca3af; cursor: not-allowed; }

.search-modal-body { flex: 1; overflow-y: auto; padding: 16px 24px; }
...
.search-result-item { display: flex; align-items: flex-start; gap: 12px; padding: 12px 14px; border: 1px solid var(--border); border-radius: 10px; margin-bottom: 8px; transition: border-color 0.12s; cursor: pointer; }
...
.search-result-url { font-size: 10px; color: var(--green); margin-top: 4px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
...
.search-modal-footer { padding: 14px 24px; border-top: 1px solid var(--border); flex-shrink: 0; display: flex; align-items: center; justify-content: space-between; background: #fafafa; }
.search-selected-count { font-size: 12px; color: var(--text-muted); }
.search-simpan-btn { padding: 9px 20px; background: var(--green); color: #fff; border: none; border-radius: 10px; font-family: 'DM Sans', sans-serif; font-size: 13px; font-weight: 600; cursor: pointer; display: flex; align-items: center; gap: 6px; }
...
.search-loading { text-align: center; padding: 40px 20px; color: var(--text-muted); }
  ...
  <div class="search-modal">
    <div class="search-modal-header">
      <div class="search-modal-title">
        ...
        <button onclick="closeSearchModal()" style="background:none;border:none;cursor:pointer;font-size:18px;color:var(--text-muted);line-height:1">×</button>
      </div>
    ...
    <div class="search-modal-body" id="search-body">
      <div class="search-empty" style="color:var(--text-muted)">
        <div style="font-size:32px;margin-bottom:10px">🔍</div>
        <div style="font-size:13px;font-weight:500">Ketik keyword lalu klik Cari</div>
        <div style="font-size:11.5px;margin-top:4px">Hasil pencarian akan muncul di sini</div>
      </div>
    </div>
...
<div class="modal-full" id="modal-konfirmasi">
  <div style="background:#fff;border-radius:18px;padding:32px 36px;width:420px;max-width:95vw;box-shadow:0 24px 64px rgba(0,0,0,0.2)">
    ...
    <div style="display:flex;gap:10px">
      <button id="btn-konfirmasi-batal" style="flex:1;padding:11px;border:1.5px solid #e5e7eb;border-radius:10px;background:#fff;font-family:'DM Sans',sans-serif;font-size:13px;cursor:pointer;color:#374151;font-weight:500">Batal</button>
      <button id="btn-konfirmasi-ulang" style="flex:1;padding:11px;border:none;border-radius:10px;background:#1e6fa3;color:#fff;font-family:'DM Sans',sans-serif;font-size:13px;cursor:pointer;font-weight:600">🔄 Generate Ulang</button>
    </div>
  </div>
</div>
...
.then(data => {
    if (data.success) {
      window.location.reload(); // ← cukup reload halaman yang sama
    } else {
      btn.disabled = false;
