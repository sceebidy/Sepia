...
<title>Infografis Intelijen — SEPIA</title>
...
<div class="main">
  <div class="topbar">📈 Infografis Intelijen</div>
  <div class="content">
    <div class="page-header"><h1>Infografis Intelijen</h1><p>Buat narasi infografis untuk visualisasi data intelijen</p></div>
    <div class="gen-layout">
      <div class="gen-card">
        <div class="gen-card-header"><span>✍️</span><span class="gen-card-title">Input Data</span></div>
        <div class="gen-card-body">
          <div class="form-group"><label class="form-label">Tema Infografis</label><input type="text" class="form-input" placeholder="cth: Peta Sebaran Kasus Korupsi Sumut 2024" /></div>
          <div class="form-group"><label class="form-label">Jenis Visualisasi</label>
            <select class="form-select"><option value="">— Pilih Jenis —</option><option>Timeline Kejadian</option><option>Peta Aktor & Jaringan</option><option>Statistik & Angka</option><option>Perbandingan Periode</option><option>Matriks Risiko</option></select>
          </div>
          <div class="form-group"><label class="form-label">Data / Fakta Utama</label><textarea class="form-textarea" placeholder="Masukkan data, angka, fakta, atau poin-poin kunci yang ingin divisualisasikan..."></textarea></div>
          <div class="form-group"><label class="form-label">Target Audiens</label>
            <select class="form-select"><option>Pimpinan / Eksekutif</option><option>Tim Operasional</option><option>Publik / Media</option><option>Lintas Instansi</option></select>
          </div>
        </div>
        <div class="gen-card-footer"><button class="btn-generate" onclick="doGenerate('ig-out','Infografis Intelijen')">⚡ Generate</button></div>
      </div>
      <div class="gen-card gen-card-output">
        <div class="gen-card-header"><span>📋</span><span class="gen-card-title">Hasil Generate</span><button class="btn-copy" onclick="doCopy('ig-out',this)">⧉ Salin</button></div>
        <div class="gen-card-body" id="ig-out">
          <div class="output-placeholder"><div class="output-placeholder-icon">📈</div><p>Narasi dan struktur infografis akan muncul di sini setelah menekan <strong>Generate</strong>.</p></div>
        </div>
      </div>
    </div>
  </div>
...
<script>
function doGenerate(id,label){const el=document.getElementById(id);el.innerHTML='<div class="output-loading"><div class="spinner"></div><span>Memproses...</span></div>';setTimeout(()=>{el.innerHTML=`<div class="output-result"><div class="output-section"><div class="output-section-title">🎨 Konsep Visual</div><p>Placeholder hasil generate <strong>${label}</strong>. Koneksi ke AI akan diaktifkan pada tahap berikutnya.</p></div><div class="output-section"><div class="output-section-title">📊 Elemen Data Utama</div><ul class="output-list"><li><strong>Angka kunci 1:</strong> Deskripsi statistik pertama yang divisualisasikan.</li><li><strong>Angka kunci 2:</strong> Deskripsi statistik kedua yang divisualisasikan.</li><li><strong>Angka kunci 3:</strong> Deskripsi statistik ketiga yang divisualisasikan.</li></ul></div><div class="output-section"><div class="output-section-title">💬 Narasi Pendamping</div><p>Sistem akan menghasilkan narasi pendamping yang ringkas, informatif, dan mudah dipahami oleh target audiens.</p></div></div>`;},1200);}
