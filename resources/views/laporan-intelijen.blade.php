...
<title>Laporan Intelijen — SEPIA</title>
...
<div class="main">
  <div class="topbar">🔍 Laporan Intelijen</div>
  <div class="content">
    <div class="page-header"><h1>Laporan Intelijen</h1><p>Buat laporan intelijen komprehensif dari data analisis</p></div>
    <div class="gen-layout">
      <div class="gen-card">
        <div class="gen-card-header"><span>✍️</span><span class="gen-card-title">Input Data</span></div>
        <div class="gen-card-body">
          <div class="form-group"><label class="form-label">Judul Laporan</label><input type="text" class="form-input" placeholder="cth: Perkembangan Jaringan Narkoba Wilayah Medan Barat" /></div>
          <div class="form-group"><label class="form-label">Kategori Intelijen</label>
            <select class="form-select"><option value="">— Pilih Kategori —</option><option>Politik, Hukum & Keamanan</option><option>Ekonomi & Keuangan</option><option>Sosial & Budaya</option><option>Ideologi & Radikalisme</option><option>Siber & Teknologi</option></select>
          </div>
          <div class="form-group"><label class="form-label">Ringkasan Temuan</label><textarea class="form-textarea" placeholder="Uraikan temuan utama, aktor, jaringan, dan modus operandi yang relevan..."></textarea></div>
          <div class="form-group"><label class="form-label">Tingkat Ancaman</label>
            <select class="form-select"><option>Rendah</option><option>Sedang</option><option>Tinggi</option><option>Kritis</option></select>
          </div>
          <div class="form-group"><label class="form-label">Wilayah</label><input type="text" class="form-input" value="Kota Medan, Sumatera Utara" /></div>
        </div>
        <div class="gen-card-footer"><button class="btn-generate" onclick="doGenerate('lint-out','Laporan Intelijen')">⚡ Generate</button></div>
      </div>
      <div class="gen-card gen-card-output">
        <div class="gen-card-header"><span>📋</span><span class="gen-card-title">Hasil Generate</span><button class="btn-copy" onclick="doCopy('lint-out',this)">⧉ Salin</button></div>
        <div class="gen-card-body" id="lint-out">
          <div class="output-placeholder"><div class="output-placeholder-icon">🔍</div><p>Laporan intelijen lengkap akan muncul di sini setelah menekan <strong>Generate</strong>.</p></div>
        </div>
      </div>
    </div>
  </div>
...
<script>
function doGenerate(id,label){const el=document.getElementById(id);el.innerHTML='<div class="output-loading"><div class="spinner"></div><span>Memproses...</span></div>';setTimeout(()=>{el.innerHTML=`<div class="output-result"><div class="output-section"><div class="output-section-title">🔖 Klasifikasi & Identitas</div><p>Placeholder hasil generate <strong>${label}</strong>. Koneksi ke AI akan diaktifkan pada tahap berikutnya.</p></div><div class="output-section"><div class="output-section-title">🔍 Analisis Ancaman</div><p>Sistem akan menghasilkan analisis intelijen terstruktur mencakup penilaian ancaman, aktor, jaringan, dan proyeksi situasi ke depan.</p></div><div class="output-section"><div class="output-section-title">⚠️ Early Warning</div><ul class="output-list"><li>Indikator peringatan dini pertama yang perlu diwaspadai.</li><li>Indikator peringatan dini kedua berdasarkan pola historis.</li></ul></div><div class="output-section"><div class="output-section-title">✅ Rekomendasi</div><ul class="output-list"><li>Rekomendasi operasional untuk satuan terkait.</li><li>Langkah koordinasi lintas instansi yang diperlukan.</li><li>Tindakan preventif jangka pendek yang disarankan.</li></ul></div></div>`;},1200);}
